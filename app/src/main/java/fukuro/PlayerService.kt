...
class PlayerService : MediaLibraryService() {
    ...
    private val systemArtwork by lazy { SystemMediaArtwork(this) }
    private val systemArtworkLoads = mutableSetOf<String>()
    ...
    /**
     * Add the square artwork after the player is active.  Server-only covers may require a
     * download, so keeping this out of playlist construction protects playback resumption's
     * three-second budget.  The generated bytes are cached by [SystemMediaArtwork].
     */
    private fun loadSystemArtworkForCurrentItem() {
        val itemId = currentItemId ?: return
        if (!systemArtworkLoads.add(itemId)) return
        scope.launch {
            try {
                val artworkData = withContext(Dispatchers.IO) {
                    systemArtwork.artworkData(itemId, fetchIfMissing = true)
                } ?: return@launch
                val current = player.currentMediaItem ?: return@launch
                if (!current.mediaId.startsWith("$BOOK_PREFIX$itemId#")) return@launch
                if (current.mediaMetadata.artworkData?.contentEquals(artworkData) == true) return@launch
                ...
                systemArtworkLoads.remove(itemId)
            }
        }
    }
    ...
    private suspend fun buildPlaylist(
        ...
        // A locally available cover can be attached right away.  A server-only cover is
        // fetched and upgraded asynchronously after playback begins (see above).
        val systemArtworkData = withContext(Dispatchers.IO) {
            systemArtwork.cachedArtworkData(itemId)
        }
        return files.map { f ->
